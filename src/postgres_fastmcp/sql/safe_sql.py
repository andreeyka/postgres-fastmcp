...
class SafeSqlConfig(SqlDriverConfig):
    """Configuration for SafeSqlDriver.

    Attributes:
        timeout: Optional timeout in seconds for query execution.
        allowed_schema: Allowed schema name (e.g., 'public'). None means all schemas are allowed.
        read_only: If True, only read operations are allowed. If False, DML (INSERT/UPDATE/DELETE)
            is allowed, but DDL is still prohibited.
        query_tag: Tag to add to SQL queries for identification in logs and monitoring.
        table_prefix: Optional table name prefix. If set and allowed_schema is set
            (user mode), only tables with names starting with this prefix are accessible.
    """
